const {
  ...
  const checkCountParentIdPegasUno = async (parentId, userId, level) => {
    if (level > 1){
      const itemsParentId = await MatrixThird.findAll({
        where: { parent_id: parentId },
      });
      if (itemsParentId.length > 1) {
        let one = await checkCountParentIdPegasUno(itemsParentId[0].id, userId, level);
        let two = await checkCountParentIdPegasUno(itemsParentId[1].id, userId, level);
      ...
      if (itemsParentId.length === 2) {
        ...
        let one = await checkCountParentIdPegasUno(itemsParentId[0].id, userId, level);
        let two = await checkCountParentIdPegasUno(itemsParentId[1].id, userId, level);
        let three = await checkCountParentIdPegasUno(itemsParentId[2].id, userId, level);
        let countOne = await MatrixThird.count({
          where: { parent_id: one.parentId },
        });
        let countTwo = await MatrixThird.count({
        ...
        let countThree = await MatrixThird.count({
  ...
  module.exports = {checkCountParentIdPegasUno}